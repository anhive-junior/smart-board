...
if ($_SESSION['uselevel'] >= 2) {
	$scr_add = "var rec = \"<td style='width:30%;'>이름</td>\"";
	$scr_add .= "rec += \"<td style='width:40%;' >MAC(IP)</td>\"";
	$scr_add .= "rec += \"<td style='width:20%;'>강퇴</td>\";";
} else {
	$scr_add = "rec += \"<td style='width:40%;'>이름</td>\"";
	$scr_add .= "rec += \"<td style='width:50%;' >MAC(IP) addr</td>\";";
} $scripts.=$scr_add;
$scripts.="
	ftr.innerHTML += rec;
	...
	for (i=0; i<resp.data.length; i++) {
		ftr = document.createElement('tr');
		var rec = \"<td>\"+resp.data[i].user+\"</td>\"+\"<td>\"+( (resp.data[i].mac!=\"\")?resp.data[i].mac:\"(\"+resp.data[i].ip+\")\" )+\"</td>\";
";
if ($_SESSION['uselevel'] >= 2) {
 $scr_add = "rec += \"<td><img src='images/cancel.png' style='width:1.2em;height:1.2em;' onclick='extract(resp.data[i].mac, resp.data[i].ip);' alt='resp.data[i].user'></td>\n\"";
} $scripts.=$scr_add;
$scripts.="
	ftr.innerHTML += rec;
	ftable.appendChild(ftr); }
